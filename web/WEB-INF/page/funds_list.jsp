                ...
                <div class="panel panel-default">
                    <div class="panel-heading">
                        <c:if test="${editEnable or (departEditEnable and (adminDepartId==projectDepartId))}">
                            <input class="btn btn-primary" type="button" value="新增" id="newBt" title="点击此处新增一条经费记录" />
                        </c:if>
                    </div>
                    <!-- /.panel-heading -->
                    <div class="panel-body">
                        <table width="100%" class="table table-striped table-bordered table-hover" id="dataTables-example">
                            ...
                            <tbody>
                            <c:forEach items="${fundsList}" var="tradeFunds" varStatus="tt">
                                <tr class="odd">
                                    <td><c:out value="${tt.index+1}"/></td>
                                    <td><c:out value="${tradeFunds.year}"/></td>
                                    <td><fmt:formatDate value="${tradeFunds.date}" dateStyle="medium" type="date"/></td>
                                    <td><p class="glyphicon-yen">&nbsp;<c:out value="${tradeFunds.funds}"/></p></td>
                                    <td><c:out value="${tradeFunds.depart}"/></td>
                                    <td>
                                        <c:if test="${editEnable or (departEditEnable and (adminDepartId==projectDepartId))}">
                                            <input class="btn btn-primary" type="button" value="编辑" id="editBt" title="edit this" onclick="doEdit(<c:out value="${tradeFunds.id}"/>)"/>
                                            <input class="btn btn-primary" type="button" value="删除" id="delBt" title="del this" onclick="doDel(<c:out value="${tradeFunds.id}"/>)"/>
                                        </c:if>
...
<script type="application/javascript">
    ...
    $("#newBt").click(function () {
        window.location.href = "edit.do?projectId=${projectId}&codeType=${codeType}";
    });
